...
/** Capacitor 8 root VC is CAPBridgeViewController; Cordova status bar used a CDVViewController category only. */
static BOOL CDVStatusBarIsCapacitorBridgeVC(UIViewController *vc) {
    static Class capBridgeClass;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        capBridgeClass = NSClassFromString(@"CAPBridgeViewController");
    });
    return capBridgeClass != nil && [vc isKindOfClass:capBridgeClass];
}

static void CDVStatusBarCapacitorSetStyle(UIViewController *vc, UIStatusBarStyle style) {
    SEL sel = NSSelectorFromString(@"setStatusBarStyle:");
    if ([vc respondsToSelector:sel]) {
        ((void (*)(id, SEL, UIStatusBarStyle))objc_msgSend)(vc, sel, style);
    }
}

static void CDVStatusBarCapacitorSetVisible(UIViewController *vc, BOOL visible) {
    SEL sel = NSSelectorFromString(@"setStatusBarVisible:");
    if ([vc respondsToSelector:sel]) {
        ((void (*)(id, SEL, BOOL))objc_msgSend)(vc, sel, visible);
    }
}
...
- (void) setStatusBarOverlaysWebView:(BOOL)statusBarOverlaysWebView
{
    ...
    if (statusBarOverlaysWebView) {
        ...
        [self initializeStatusBarBackgroundView];
        [self.viewController.view addSubview:_statusBarBackgroundView];
...
- (void) setStyleForStatusBar:(UIStatusBarStyle)style
{
    if (_uiviewControllerBasedStatusBarAppearance) {
        ...
        if (CDVStatusBarIsCapacitorBridgeVC(vc)) {
            CDVStatusBarCapacitorSetStyle(vc, style);
        } else if ([vc isKindOfClass:[CDVViewController class]]) {
            CDVViewController *cdv = (CDVViewController *)vc;
            cdv.sb_statusBarStyle = [NSNumber numberWithInt:(int)style];
...
- (void) hideStatusBar
{
    if (_uiviewControllerBasedStatusBarAppearance) {
        UIViewController *vc = self.viewController;
        if (CDVStatusBarIsCapacitorBridgeVC(vc)) {
            CDVStatusBarCapacitorSetVisible(vc, NO);
        } else if ([vc isKindOfClass:[CDVViewController class]]) {
            CDVViewController *cdv = (CDVViewController *)vc;
            cdv.sb_hideStatusBar = [NSNumber numberWithBool:YES];
            [self refreshStatusBarAppearance];
        } else {
            [self refreshStatusBarAppearance];
        }
...
- (void) showStatusBar
{
    if (_uiviewControllerBasedStatusBarAppearance) {
        UIViewController *vc = self.viewController;
        if (CDVStatusBarIsCapacitorBridgeVC(vc)) {
            CDVStatusBarCapacitorSetVisible(vc, YES);
        } else if ([vc isKindOfClass:[CDVViewController class]]) {
            CDVViewController *cdv = (CDVViewController *)vc;
            cdv.sb_hideStatusBar = [NSNumber numberWithBool:NO];
            [self refreshStatusBarAppearance];
        } else {
            [self refreshStatusBarAppearance];
        }
